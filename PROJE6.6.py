...
class HumanPlayer(Player):
    def make_move(self):
        move = input("Kaya, Kağıt, veya Makas seçin: ").lower()
        while move not in ['kaya', 'kağıt', 'makas']:
            move = input("Geçersiz seçim! Kaya, Kağıt, veya Makas seçin: ").lower()
        return move

class RandomComputerPlayer(ComputerPlayer):
    def make_move(self):
        return random.choice(['kaya', 'kağıt', 'makas'])
...
class Game:
    ...
    def get_winner(self, human_move, computer_move):
        ...
        elif (human_move == 'kaya' and computer_move == 'makas') or \
             (human_move == 'kağıt' and computer_move == 'kaya') or \
